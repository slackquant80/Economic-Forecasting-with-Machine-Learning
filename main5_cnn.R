...
run_multi_cnn=function(Y,indice,lag){
  ...
  model %>% 
    layer_conv_1d(filters = 32, kernel_size = 2, activation = "relu", input_shape = c(4, feature)) %>% 
...
# ============================================================================
# Multivariate Rolling 1 Step ahead CNN Forecast

mul.cnn.rolling.window=function(Y,nprev,indice=1,lag=1){
  
  save.pred=matrix(NA,nprev,1)
  
  for(i in nprev:1){
    Y.window=Y[(1+nprev-i):(nrow(Y)-i),] %>% as.data.frame()
    cnn=run_multi_cnn(Y.window,indice,lag)
    save.pred[(1+nprev-i),]=as.numeric(cnn$pred) # Note as.numeric()
...
cnn_1 <- mul.cnn.rolling.window(Y,nprev,1,1)  # forecasting horizon 설정
cnn_3 <- mul.cnn.rolling.window(Y,nprev,1,3)
cnn_6 <- mul.cnn.rolling.window(Y,nprev,1,6)
cnn_12 <- mul.cnn.rolling.window(Y,nprev,1,12)

# Get the error
cnn.rmse = cbind(cnn_1$errors[1], cnn_3$errors[1], cnn_6$errors[1], cnn_12$errors[1])
cnn.mae = cbind(cnn_1$errors[2], cnn_3$errors[2], cnn_6$errors[2], cnn_12$errors[2])

# Get the prediction
cnn.pred = cbind(cnn_1$pred, cnn_3$pred, cnn_6$pred, cnn_12$pred)
...
save.image("results_forecasts(5).RData")
